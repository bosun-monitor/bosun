...
func Expr(t miniprofiler.Timer, w http.ResponseWriter, r *http.Request) (v interface{}, err error) {
	...
	tsdbs := &expr.TSDBs{
		...
		Azure:      schedule.SystemConf.GetAzureMonitorContext(),
		Prometheus: schedule.SystemConf.GetPromContext(),
	}
	...
	res, queries, err := e.Execute(tsdbs, providers, t, now, 0, false, "Web: expression execution")
	if err != nil {
		return nil, err
	}
