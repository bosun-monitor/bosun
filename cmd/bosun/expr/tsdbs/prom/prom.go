...
func RawAggSeriesQuery(prefix string, e *expr.State, query, stepDuration, sdur, edur string) (*expr.ValueSet, error) {
...
func MRawAggSeriesQuery(prefix string, e *expr.State, query, stepDuration, sdur, edur string) (*expr.ValueSet, error) {
...
func rawAggSeriesQuery(prefix string, e *expr.State, query, stepDuration, sdur, edur string, multi bool) (r *expr.ValueSet, err error) {
	r = new(expr.ValueSet)
...
func Query(prefix string, e *expr.State, metric, groupBy, filter, agType, stepDuration, sdur, edur string) (r *expr.ValueSet, err error) {
...
func Rate(prefix string, e *expr.State, metric, groupBy, filter, agType, rateDuration, stepDuration, sdur, edur string) (r *expr.ValueSet, err error) {
...
func MQuery(prefix string, e *expr.State, metric, groupBy, filter, agType, stepDuration, sdur, edur string) (r *expr.ValueSet, err error) {
...
func MRate(prefix string, e *expr.State, metric, groupBy, filter, agType, rateDuration, stepDuration, sdur, edur string) (r *expr.ValueSet, err error) {
...
func mQuery(prefix string, e *expr.State, metric, groupBy, filter, agType, rateDuration, stepDuration, sdur, edur string) (r *expr.ValueSet, err error) {
	r = new(expr.ValueSet)
	prefixes := strings.Split(prefix, ",")
	...
	wg := sync.WaitGroup{}
	wg.Add(len(prefixes))
	resCh := make(chan *expr.ValueSet, len(prefixes))
	errCh := make(chan error, len(prefixes))
	...
	resultCollection := []*expr.ValueSet{}
	for res := range resCh {
...
func buildAndQuery(prefix string, e *expr.State, metric, groupBy, filter, agType, rateDuration, stepDuration, sdur, edur string, addPrefixTag bool) (r *expr.ValueSet, err error) {
	r = new(expr.ValueSet)
	start, end, err := expr.ParseDurationPair(e, sdur, edur)
	if err != nil {
		return
...
func matrixToResults(prefix string, e *expr.State, res promModels.Value, expectedTagLen int, addPrefix bool, r *expr.ValueSet) (err error) {
...
func MetricList(prefix string, e *expr.State) (r *expr.ValueSet, err error) {
	r = new(expr.ValueSet)
	client, found := e.Prometheus[prefix]
	if !found {
...
func TagInfo(prefix string, e *expr.State, metric, sdur, edur string) (r *expr.ValueSet, err error) {
	r = new(expr.ValueSet)
	client, found := e.Prometheus[prefix]
	if !found {
		return r, fmt.Errorf(`prometheus client with name "%v" not defined`, prefix)
