...
// InitClient2 sets up the elastic client. If the client has already been
// initialized it is a noop
func InitClient2(prefix string, esHosts tsdbs.ElasticHosts) error {
...
// getService2 returns an elasticsearch service based on the global client
func getService2(prefix string, esHosts tsdbs.ElasticHosts) (*elastic.SearchService, error) {
	esClients.Lock()
...
// elasticsearch.
func Query2(r *ElasticRequest2, esHosts tsdbs.ElasticHosts) (*elastic.SearchResult, error) {
	s, err := getService2(r.HostKey, esHosts)
	if err != nil {
		return nil, err
	}
...
// the search results.
func timeESRequest2(e *expr.State, req *ElasticRequest2) (resp *elastic.SearchResult, err error) {
	...
	e.Timer.StepCustomTiming("elastic", "query", fmt.Sprintf("%s:%v\n%s", req.HostKey, req.Indices, b), func() {
		getFn := func() (interface{}, error) {
			return Query2(req, e.Backends.ElasticHosts)
		}
		var val interface{}
		var hit bool
...
func ESDateHistogram2(prefix string, e *expr.State, indexer tsdbs.ESIndexer, keystring string, filter elastic.Query, interval, sduration, eduration, stat_field, rstat string, size int) (r *expr.Results, err error) {
	r = new(expr.Results)
...
func ESBaseQuery2(now time.Time, indexer tsdbs.ESIndexer, filter elastic.Query, sduration, eduration string, size int, prefix string) (*ElasticRequest2, error) {
