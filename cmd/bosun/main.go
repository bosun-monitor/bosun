...
func main() {
	...
	go func() {
		slog.Fatal(web.Listen(sysProvider.GetHTTPListen(), sysProvider.GetHTTPSListen(), sysProvider.GetTLSCertFile(), sysProvider.GetTLSKeyFile(), *flagDev, sysProvider.GetTSDBHost(), reload, sysProvider.GetAuthConfig()))
	}()
	go func() {
		if !*flagNoChecks {
			sched.Run()
