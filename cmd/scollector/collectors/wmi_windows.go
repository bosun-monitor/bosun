...
func wmiParseCIMDatetime(cimdatetime string) (time.Time, error) {
	...
	if i < 0 {
		return time.Unix(0, 0).UTC(), fmt.Errorf("Invalid CIM_DATETIME format, cannot find UTC offset.")
	}
	t, err := time.Parse("20060102150405", cimdatetime[0:i])
	if err != nil {
		return time.Unix(0, 0).UTC(), err
	}
	offset, err := time.ParseDuration(fmt.Sprintf("%vm", cimdatetime[i:len(cimdatetime)]))
	if err != nil {
		return time.Unix(0, 0).UTC(), err
	}
