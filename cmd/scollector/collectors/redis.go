...
func c_redis(r conf.RedisRemote) (opentsdb.MultiDataPoint, error) {
	...
	for _, line := range infoSplit {
		...
		if strings.HasPrefix(sp[0], "cmdstat_") {
			...
			cmdStatsCallsSplit := strings.Split(cmdStats[0], "=")
			if len(cmdStatsCallsSplit) < 2 {
				continue
			}
			cmdStatsUsecSplit := strings.Split(cmdStats[1], "=")
			if len(cmdStatsUsecSplit) < 2 {
				continue
			}
			cmdStatsUsecPcSplit := strings.Split(cmdStats[2], "=")
			if len(cmdStatsUsecPcSplit) < 2 {
				continue
			}
			...
			Add(&md, "redis.cmdstats_calls", cmdStatsCallsSplit[1], tags, metadata.Counter, metadata.Operation, descRedisCmdCalls)

			if r.UseNano {
				cmdStatsUsec, err := strconv.ParseInt(cmdStatsUsecSplit[1], 10, 64)
				if err != nil {
					continue
				}

				Add(&md, "redis.cmdstats_usec", cmdStatsUsec, tags, metadata.Counter, metadata.Nanosecond, descRedisCmdSec)

				if r.PerCallStats {
					cmdStatsUsecPc, err := strconv.ParseFloat(cmdStatsUsecPcSplit[1], 64)
					if err != nil {
						continue
					}

					Add(&md, "redis.cmdstats_usec_pc", cmdStatsUsecPc, tags, metadata.Gauge, metadata.MilliSecond, descRedisCmdSecPc)
				}
			} else {
				var cmdStatsMsec, cmdStatsMsecPc float64
				microsec, err := strconv.ParseFloat(cmdStatsUsecSplit[1], 64)
				if err != nil {
					continue
				}
				cmdStatsMsec = microsec / 1000

				Add(&md, "redis.cmdstats_msec", cmdStatsMsec, tags, metadata.Counter, metadata.MilliSecond, descRedisCmdSec)
				...
				if r.PerCallStats {
					...
					microsec, err = strconv.ParseFloat(cmdStatsUsecPcSplit[1], 64)
					if err != nil {
						continue
					}
					cmdStatsMsecPc = microsec / 1000

					Add(&md, "redis.cmdstats_msec_pc", cmdStatsMsecPc, tags, metadata.Gauge, metadata.MilliSecond, descRedisCmdSecPc)
				}
			}
			continue
		}
...
const (
	...
	descRedisCmdSecPc = "The average CPU consumed per command execution."
	descRedisCmdSec   = "The total CPU time consumed by commands."
